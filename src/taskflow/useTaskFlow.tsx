...
export function useTaskFlow(opts: UseTaskFlowOptions = {}) {
  ...
  const acceptProjectInviteMutation = useMutation({
    mutationFn: ({ workspaceSlug, projectId }: { workspaceSlug: string; projectId: string }) =>
      projectMembersApi.acceptInvite(workspaceSlug, projectId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['workspaces', wsSlug, 'projects'] });
      qc.invalidateQueries({ queryKey: ['ws', wsSlug, 'notifications'] });
    },
  });

  const declineProjectInviteMutation = useMutation({
    mutationFn: ({ workspaceSlug, projectId }: { workspaceSlug: string; projectId: string }) =>
      projectMembersApi.declineInvite(workspaceSlug, projectId),
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ['ws', wsSlug, 'notifications'] }),
      qc.invalidateQueries({ queryKey: ['workspaces', wsSlug, 'projects'] });
    },
  ...
  const notifVerb: Record<ApiNotification['type'], string> = {
    ...
    project_invite_received: 'invited you to project',
  };
  ...
  const inboxItems = (notifQ.data?.data ?? []).map((n) => {
    ...
    return {
      ...
      target: loc
        ? loc.key
        : n.type === 'project_invite_received' || n.type === 'project_member_added'
        ? 'a project'
        : n.issue_id
        ? 'an issue'
        : 'your workspace',
